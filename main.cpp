...
public:
  ...
  Framework(bool visible_, int height_, int width_) : visible(visible_), height(height_), width(width_) {
    if (visible) {
  ...
  void refresh() {
    ...
    // batteryController.percentRemaining
    {
      ...
      SDL_Rect rect {.x = center_x - rect_height / 2, .y = center_y, .w = max_bar_length, .h = rect_height};
      SDL_SetRenderDrawColor(renderer, 128, 128, 128, 255);
  ...
  void set_forecast(uint64_t timestamp,
                    std::array<std::optional<Pinetime::Controllers::SimpleWeatherService::Forecast::Day>,
  ...
  void generate_weather_data(bool clear) {
    if (clear) {
      ...
      std::array<std::optional<Pinetime::Controllers::SimpleWeatherService::Forecast::Day>,
                 Pinetime::Controllers::SimpleWeatherService::MaxNbForecastDays>
        days;
      set_forecast(0, days);
      return;
    }
